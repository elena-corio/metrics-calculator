...
def calculate_green_space_index_avg(res_units: list[Unit], green_spaces: list[OpenSpace], target: float) -> float:
    """
    Calculate avg green space index score for a list of units.
    """
    total_score = sum(calculate_green_space_index(unit, green_spaces, target) for unit in res_units)
